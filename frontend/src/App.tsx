...
import {
  LayoutDashboard, Activity, FlaskConical, ListChecks, History, Zap, Shield, BarChart3, Settings, ExternalLink, Download, Bell, ShoppingCart, Key, ShieldCheck, ArrowRight, AlertTriangle
} from 'lucide-react';
import { Badge, Panel, Stat, NavItem } from './components/Common';
import { workstationService } from './services/api';
import type { Symbol, Signal, QueueItem, Position, Experiment, Alert, BrokerAccount, SystemSettings } from './types';

const Toast = ({ message, type, onClose }: { message: string, type: 'success' | 'error', onClose: () => void }) => (
  <div className={`fixed bottom-8 right-8 z-[100] p-4 rounded-xl shadow-2xl border flex items-center gap-3 animate-in slide-in-from-right duration-300 ${
    type === 'success' ? 'bg-success/10 text-success border-success/20' : 'bg-danger/10 text-danger border-danger/20'
  }`}>
...
const App = () => {
  ...
  const [challenger, setChallenger] = useState<Experiment | null>(null);

  const fetchData = async () => {
    try {
      const [symRes, sigRes, qRes, posRes, expRes, setRes, anaRes, alRes, bkRes] = await Promise.all([
        workstationService.getSymbols(),
        workstationService.getSignals(),
        workstationService.getQueue(),
        workstationService.getPositions(),
        workstationService.getExperiments(),
        workstationService.getSettings(),
        workstationService.getAnalytics(),
        workstationService.getAlerts(),
        workstationService.getBrokerAccounts()
      ]);
  ...
  const toggleKillSwitch = async () => {
    try {
      const res = await workstationService.toggleKillSwitch();
      showToast(`Kill Switch ${res.data.kill_switch ? 'ACTIVATED' : 'DEACTIVATED'}`, res.data.kill_switch ? 'error' : 'success');
      fetchData();
    } catch (e) { showToast('Failed to toggle kill switch', 'error'); }
  };

  const exportState = async () => {
     try {
       const res = await workstationService.exportState();
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <main className="flex-1 overflow-y-auto p-8">
          <div className="max-w-6xl mx-auto">
                ...
                <Panel title="Lab Configuration">
                  <div className="grid grid-cols-4 gap-4">
                    ...
                    <div className="space-y-2">
                       ...
                       <select className="w-full bg-background border border-border rounded px-3 py-2 text-xs" id="scan_mode_selector" onChange={(e: any) => {
                    ...
                    <div className="space-y-2">
                       ...
                       <select className="w-full bg-background border border-border rounded px-3 py-2 text-xs" id="tf_selector">
                          <option>M5</option><option>M15</option><option value="H1">H1</option><option>H4</option>
                       </select>
                    </div>
                  </div>
                       ...
                       <button onClick={async () => {
                          ...
                          try {
                            if (scanMode === 'Auto Best Setup') {
                               await workstationService.runAutoBest('EURUSD', 'M15,H1,H4');
                            } else {
                               await workstationService.runScan('EURUSD', tf);
                            }
                    ...
                    <tbody className="divide-y divide-border/30">
                      {signals.length === 0 ? (
                        ...
                        <tr key={s.id} className="hover:bg-secondary/10 transition-colors">
                          ...
                          <button onClick={async () => {
                             try {
                               await workstationService.addToQueue(s.id);
                               showToast('Signal added to queue');
                               fetchData();
                               setActiveTab('queue');
                             } catch (e) { showToast('Failed to add to queue', 'error'); }
                          }} className="bg-primary/5 text-primary border border-primary/20 px-4 py-2 rounded-md font-black uppercase text-[10px] hover:bg-primary hover:text-white transition-all shadow-sm">Add to Queue</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
                    ...
                    {queue.filter(q => q.status === 'active').map(q => (
                      <div key={q.id} className="flex items-center justify-between p-5 bg-card rounded border border-border hover:border-primary/40 transition-all shadow-sm group">
                        ...
                        <div className="flex gap-2">
                          <button onClick={async () => {
                             try {
                               const res = await workstationService.executeTrade(q.id);
                               if (res.data.retcode === 10009 || res.data.retcode === 10008) {
                                  showToast('Trade executed successfully');
                               } else {
                                  showToast(`Execution failed: ${res.data.comment || 'Unknown'}`, 'error');
                               }
                               fetchData();
                             } catch (e) { showToast('Network error during execution', 'error'); }
                          }} disabled={q.status !== 'active' || settings?.kill_switch} className="bg-primary px-8 py-3 rounded-md text-[11px] font-black uppercase tracking-widest shadow-lg shadow-primary/20 hover:scale-[1.02] transition-all">Execute Trade</button>
                        </div>
                      </div>
                    ))}
                 </div>
                    ...
                    <div className="grid grid-cols-2 gap-6">
                       {['ema_pullback', 'breakout_retest'].map(sid => (
                          <div key={sid} className="p-6 bg-secondary/10 border border-border rounded-xl flex flex-col justify-between group hover:border-primary/50 transition-all">
                             ...
                             <button onClick={async () => {
                                try {
                                   await workstationService.runBacktest(sid, 'EURUSD');
                                   showToast('Backtest triggered');
                                   fetchData();
                                } catch (e) { showToast('Backtest failed', 'error'); }
                             }} className="w-full bg-primary py-3 rounded-lg font-black uppercase text-[11px] tracking-widest">Run Validation</button>
                          </div>
                       ))}
                    </div>
                 </Panel>
                        ...
                        {experiments.map(e => (
                          <tr key={e.id} className="hover:bg-secondary/10 transition-colors">
                            ...
                            <td className="py-5 px-3 text-right">
                               ...
                               <button onClick={async () => {
                                  try {
                                     await workstationService.promoteExperiment(e.id);
                                     showToast(e.is_active ? 'Preset deactivated' : 'Preset promoted');
                                     fetchData();
                                  } catch (e) { showToast('Promotion failed', 'error'); }
                               }} className={`px-3 py-1.5 rounded font-black uppercase text-[9px] border ${e.is_active ? 'bg-success/10 text-success' : 'bg-primary/10 text-primary'}`}>
                    ...
                    <div className="fixed inset-0 bg-background/90 backdrop-blur-md z-50 flex items-center justify-center p-8">
                       <div className="bg-card border border-border rounded-2xl w-full max-w-4xl shadow-2xl overflow-hidden">
                          ...
                          <div className="p-8 bg-secondary/10 border-t border-border flex justify-center gap-6">
                             ...
                             <button onClick={() => { workstationService.promoteExperiment(challenger.id).then(() => { fetchData(); setChallenger(null); }); }} className="px-12 py-4 bg-primary text-white rounded-xl font-black uppercase text-xs tracking-widest shadow-xl shadow-primary/20 flex items-center gap-3">
            ...
            {activeTab === 'alerts' && <div className="space-y-3">{alerts.map(a => <div key={a.id} className="p-4 bg-secondary/10 border border-border rounded-lg flex justify-between items-center"><div><Badge status={a.level === 'critical' ? 'blocked' : 'research_only'}>{a.level}</Badge><span className="ml-3 font-bold">{a.title}</span><p className="text-xs text-muted-foreground mt-1">{a.message}</p></div><button onClick={() => workstationService.markAlertRead(a.id).then(fetchData)} className="text-[9px] font-black uppercase">Read</button></div>)}</div>}
            {activeTab === 'journal' && <div className="space-y-4">{positions.filter(p => p.status === 'closed').map(p => <div key={p.id} className="p-5 bg-card border border-border rounded-lg flex justify-between items-center cursor-pointer" onClick={() => setSelectedPosition(p)}><div className="flex gap-4"><div className={`w-1 h-8 rounded-full ${p.pnl >= 0 ? 'bg-success' : 'bg-danger'}`}></div><div><div className="font-black uppercase tracking-tight">{p.symbol}</div><div className="text-[10px] text-muted-foreground">{p.created_at}</div></div></div><div className={`font-black ${p.pnl >= 0 ? 'text-success' : 'text-danger'}`}>{p.pnl >= 0 ? '+' : ''}${p.pnl.toFixed(2)}</div></div>)}</div>}
            {activeTab === 'brokers' && (
                    ...
                    <div className="space-y-4">
                       {brokerAccounts.map(acc => (
                          <div key={acc.id} className="p-4 bg-secondary/10 border border-border rounded-lg flex justify-between items-center">
                             ...
                             <div className="flex gap-2"><button onClick={() => workstationService.testConnection(acc.id).then(r => alert(JSON.stringify(r.data)))} className="bg-primary/10 text-primary border border-primary/20 px-3 py-1 rounded text-[10px] font-black uppercase">Test</button><Badge status={acc.is_active ? 'live_ready' : 'used'}>{acc.is_active ? 'Active' : 'Disabled'}</Badge></div>
                          </div>
                       ))}
                    </div>
                 </Panel>
                 <div className="bg-secondary/5 border border-border rounded-xl p-8 space-y-4">
                    ...
                    <div className="space-y-4">
                       ...
                       <button onClick={async () => {
                          ...
                          try {
                            await workstationService.linkAccount({ broker_name: 'Exness', login: parseInt(login), password: pwd, server: srv, is_active: true });
                            showToast('Account linked successfully');
                            fetchData();
                          } catch (e) { showToast('Failed to link account', 'error'); }
